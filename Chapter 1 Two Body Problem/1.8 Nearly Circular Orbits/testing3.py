...
for i in range(len(time)):
    ...
    x1[i] = (diff_x + orbit_radius) * np.cos(phi)
    x2[i] = (diff_x + orbit_radius) * np.sin(phi)
    x3[i] = diff_z
...
ani = FuncAnimation(fig, update, frames=len(time), init_func=init, interval=10, blit=True)
# ani.save(filename="assets/bad_sim.gif", writer="pillow")
plt.show()
