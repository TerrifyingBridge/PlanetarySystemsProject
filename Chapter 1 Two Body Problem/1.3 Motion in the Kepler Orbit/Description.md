...
### Vector3D
Since I wanted to make this project in a 3D coordinate system, I needed to update my vector class to the new dimension. Most of this method is exactly what you would expect, just adjusting for the $z$ term for 3D. However, there are a few interesting parts I wanted to discuss.
...
Where $\mathbf{r_{0}}$ is the initial position of the partial and $\mathbf{v_{0}}$ is the initial velocity of the particle. Using this definition, many other relations found earlier in the book, and a lot of algebraic manipulation, one can find Gauss's functions in terms of the true anomaly and eccentric anomaly. Since the eccentric anomaly (designated using the letter $u$) is the easiest one that directly correlates to time, this is the variation I used. The book gives us this equation, but just to be certain I understood what was happening, I made sure to do it myself as well (for my worked out solution to this, see Problem 12 in the Chapter Problems file). These equations ended up being the following.
...
7. Use Kepler's Equation to determine the initial mean anomaly $l_{0}$
...
The book mentioned that Kepler's equation cannot be solved analytically, and needs to be done numerically. The solution I ended up using was fixed-point iteration. I saw that technically Newton's method converged faster, but Wikipedia said that fixed-point iteration was identical to Kepler's solution, so I stuck with that. Here is the code for it.
...
As seen from the picture, there is *some* deviation that at least seems to resemble a hyperbolic motion, although I did not verify this outside of an eye test. I am not entirely sure why this particular method does not work. Just to be certain I was doing the process correctly, I kept the same gross un-simplified Gauss's function and plugged in the relations for elliptical orbits instead. Upon doing so, nothing changed between this and what I was doing before. I am sure I missed something, but I am unsure what it is. Because it is so similar to a hyperbolic orbit, just really stretched, I am fine leaving it as it is.
...
I found this section to be significantly more digestible than the previous section. In the previous section, I really felt like the jumps in the steps were a lot harder to grasp. However, this section connected a lot better in terms of what was being demonstrated. I imagine this is one of three things: the book is getting better at representing its material, I'm getting less rusty at the physics behind the skips so it's easier to follow, or the book is building on itself rather than assumed knowledge so there are less gaps for me to need to fill in. I believe it's probably the latter 2, but I wouldn't call this book perfect either. There are many times where the book mentions "this is the case" for certain explanations, and I'm sure this is because the book is advertised for "advanced undergrads, graduate students, and researchers new to the field" (as per the back of the book), but it still annoys me. The other thing I want to point out is the notation, as I don't exactly know how standard it is. Whenever I am confused on a topic, I look up reference material to help work me through it, and when doing so, the notation is always different. For example, in the book, Kepler's equation is $\ell = u - e\sin(u)$ while in most resources online I found it was written as $M = E - e\sin(E)$. I suppose this isn't a huge deal, but something I've noticed. Anyway, into the coding project.

### Self Imposed Exercises Thoughts
Alright, so I probably didn't need to do *all* of the orbital averages in this section, especially since there are even more in the practice problems for this book. The first few were alright, but after around five or so I started to get over it. Mostly because after the 5th one, it started to get a bit redundant and same-y. I think you can see it in my representation of each problem as I really shorten how much work is shown and jumps I take myself. It really makes me think about all the times I've complained about jumps the book takes, that have left me confused, which pretty much confirms the fact that the problem is my own rustiness with the material rather than the book's representation. One thing I don't entirely understand is where these specific orbital averages are used. Some I could understand, but I don't see the importance of $\langle (a / r) ^{3} \cos(f)\sin(f) \rangle$. Don't get me wrong, I believe an application exists, but I don't know it yet. 

### Project Thoughts
This was a very fulfilling project, and I was really excited that I got to make it work as well as it did. It's skeleton structure was very similar to the project in Section 1.2, but I was so excited to implement everything. I haven't played around with 3D graphs in matplotlib before, so it was fun being able to learn how to do that, even though it didn't take too long to do so. I was also surprised with how well the elliptical orbits worked out. They all looked very smooth and, by the end, were very nice to look at. There were a number of times I put in random numbers into the program to see what sorts of orbits I would see.

This was the biggest reason I wanted to add hyperbolic orbits. The original plan didn't have hyperbolic orbits, but I wanted to add it because the program kept crashing when the eccentricity was greater than 1 (or equal to). However, now that I've implemented the hyperbolic orbit, I am really upset that I wasn't able to make it work fully, especially since I am not entirely sure what is wrong. I am sure there is some simplification I messed up with whatever, but I'll have to leave it as is. Of course, having hyperbolic AND elliptical orbits begs the question of why didn't I also do parabolic orbits. This is mainly because I have never accidently stumbled upon a parabolic orbit (since we need eccentricity to be exactly 1, which is really hard with floating point error). I suppose I could have done it, but didn't really want to put in the effort, especially since the hyperbolic orbit didn't work.

Playing around with the program also made me realize there are some other errors with the code too. A great example of this is that when the velocity of the test particle is parallel with the position vector. In these cases, you might expect the particle to either move away from the mass or crash directly into it (depending on the magnitude and direction of the velocity). In my program, it just crashes, which happens because the eccentricity is exactly 1 and thus gives a lot of undefined values due to the $(1 - e^{2})$ term. Another example of the code not entirely working properly is closing the program. For some reason, when I closed the GUI window when running the animation, the program would take a long while to actually close. I am not entirely sure what caused this, but if I had to guess, it has something to do with not hitting the segment of code that is meant to break the mainloop for Tkinter. 

Overall, there is a bunch of stuff to fix in this program that I wasn't able to fully implement. If I were to work on this project more, I would have ironed it out, but I need to move on to the next project. Similarly, there are a bunch of non-bug fixes I wanted to implement, such as a cleaner GUI or adding important information to the user such as telling them the eccentricity of the orbit before plotting it. Regardless of this, I am really happy with how it turned out, and the elliptical orbits look pretty and really cool, especially being able to start at any point in the orbit instead of just the periapsis or apoapsis like the previous project.

### Concluding Thoughts
Overall, I had a lot of fun with this section. There was a lot to do, and I feel like a lot of what I did here came naturally. Choosing the exercises felt good, and finding the project came rather easy to me as well. Like the last sections, I feel like I learned a lot more than what I was expecting. All of this material is all Chapter 1 stuff, so I shouldn't expect anything too revolutionary, but so far I've left each section feeling like I am a huge step forward in the subject (even if I have a *long* way to go). It feels especially silly, since I had learned material similar to this in my physics classes back in undergrad, but I haven't felt like I'd learned it nearly as well. I am a little bummed that I wasn't able to get the hyperbolic orbit working, but I really needed to call the project quits before I got too carried away. If I didn't have a whole book to get through, then I would have stayed and really grinded away at it, but when the goal is to have like 100 small projects, I need to remember when I've spent too long on one.

Another important takeaway from this section was it was the first section I had to work on while my job was taking place. Sections 1.1 and 1.2 happened during Thanksgiving break and therefore meant I had a lot of time to work on it. This one, however, I had to work on after I got home from my job, which wasn't too often. Because of this, this section took around 2 weeks, while each section before only took around 1 day. Moving forward, I am going to need to focus on when I'm spending too much time on one project. I really want to be able to get through this book, so I need to learn what my balance is between quality and quantity. I was also looking ahead in the next two sections, and I am not entirely sure how I am going to make a coding project out of it. Both of them seem more math and clarifiction heavy rather than concepts that can be represented with code. I am sure I'll figure it out, but I bet those projects will be a bit of a stretch for their sections and will be a bit shorter. 
